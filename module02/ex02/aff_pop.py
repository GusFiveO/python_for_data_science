...
def aff_pop():
	...
	population_total = population_total.select(["country"] + [str(year) for year in range(1800, 2050)])

	population_total_by_country = population_total.rows_by_key(key=["country"], named=True)

	years_, france_population = zip(*population_total_by_country["France"][0].items())
	years_, belgium_population = zip(*population_total_by_country["Belgium"][0].items())
